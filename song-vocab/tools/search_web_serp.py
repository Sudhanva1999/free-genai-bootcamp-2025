...
async def search_web_serp(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    """
    Search the web for Marathi song lyrics using SERP API.
    ...
    # Add Marathi-specific keywords to improve results
    marathi_keywords = ["गाणी", "lyrics", "मराठी"]
    enhanced_query = f"{query} {' '.join(marathi_keywords)}"
    ...
    try:
        ...
        params = {
            ...
            "q": enhanced_query,
            "num": max_results,
            "hl": "mr",  # Marathi language results
            "gl": "in",  # Results from India
            "api_key": api_key
        }
