...
async def extract_vocabulary(text: str) -> List[dict]:
    """
    Extract ALL vocabulary from Marathi text using LLM with structured output.
    
    Args:
        text (str): The text to extract vocabulary from
        
    Returns:
        List[dict]: Complete list of vocabulary items in Marathi format with Devanagari, phonetic transcription, and parts
    """
    ...
    try:
        ...
        prompt_path = Path(__file__).parent.parent / "prompts" / "Extract-Marathi-Vocabulary.md"
